...
/// Every shipped design must survive the path the app puts it through. Both
/// halves are asserted because a design can fail in two very different ways:
/// markers that no longer detect (a quantiser shifting one channel is enough),
/// and markers that detect but land somewhere the content does not fit.
///
/// Driven off `BUILTIN_DESIGNS` rather than a hardcoded list, so adding a
/// design to the picker cannot skip this. They are generated by
/// `assets/art/generate_bill_art.py`; if that script changes, this is what
/// catches a design that stopped being usable.
#[test]
fn every_builtin_design_composes_on_every_variant() {
    assert!(
        template::BUILTIN_DESIGNS.len() >= 3,
        "expected the three shipped designs"
    );
    assert!(
        template::default_design().is_default()
            && template::default_design().as_template().is_none(),
        "the default design must compose through the built-in path, not detection"
    );

    for design in template::BUILTIN_DESIGNS {
        assert_eq!(
            template::builtin_design(design.id).map(|d| d.name),
            Some(design.name),
            "{}: not reachable by id",
            design.id
        );

        // Marker designs must detect to exactly the layout the built-in uses.
        if let Some(png) = design.as_template() {
            let img = image::load_from_memory(png)
                .unwrap_or_else(|e| panic!("{}: decode: {e}", design.id))
                .to_rgb8();
            let spec = TemplateSpec::detect(&img)
                .unwrap_or_else(|e| panic!("{}: detect: {e}", design.id));
            assert_eq!(
                spec,
                template::satoshi_spec(),
                "{} detects a layout other than the built-in one",
                design.id
            );
        }
        ...
        for variant in [Variant::Segwit, Variant::Taproot, Variant::TaprootBackup] {
            let wallet = wallet_for(variant);
            let png = match design.as_template() {
                Some(tpl) => template::compose_custom_bill(tpl, &wallet, TS)
                    .unwrap_or_else(|e| panic!("{}/{variant:?}: compose: {e}", design.id)),
                None => bill::compose_bill(&wallet, YEAR, TS)
                    .unwrap_or_else(|e| panic!("{}/{variant:?}: compose: {e}", design.id)),
            };
            let payloads = decode_qrs(&png);
            assert!(
                payloads.contains(&qr::address_payload(&wallet.address)),
                "{}/{variant:?}: address QR did not decode back to the address",
                design.id
            );
            assert!(
                payloads.contains(&qr::sweep_url(&wallet.bill_wif, wallet.variant)),
                "{}/{variant:?}: private-key QR did not decode back to the sweep URL",
                design.id
            );
        }
    }
